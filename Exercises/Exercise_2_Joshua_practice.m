func2 =@(tau,tvec) heaviside(tau/2+tvec) - heaviside(tvec-tau/2);

T_symb = 0.025;
BW = 1/(2*T_symb); % one-sided bandwidth
...
p_samp = func2(T_symb,tvec);
p_continuous = func2(T_symb,cont_tvec);
...
stem(tvec,p_samp);
xlabel('t(s)')
legend('cintinuous signal','Sampled signal')
...
title(['Frequence response of sampled signal, f_{samp}/BW= ',num2str(f_samp/BW)])
...
title(['Phase response of a sampled signal, f_{samp}/BW= ',num2str(f_samp/BW)]);
